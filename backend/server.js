...
const apiKey = process.env.DEMO_MODE === "true" ? "" : process.env.GHUB_API_KEY;
...
const fallbackTrailerPreview =
  "https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&w=1600&q=80";
const fallbackDownloadWebsite = {
  id: 0,
...
const downloadWebsites = [
  ...
  },
  {
    ...
    urlFor(game) {
      return `https://store.epicgames.com/browse?q=${encodeURIComponent(game.name)}&sortBy=relevancy`;
    },
  },
  {
    id: 8,
    ...
    domain: "gog.com",
    urlFor(game) {
      return `https://www.gog.com/en/games?query=${encodeURIComponent(game.name)}`;
    },
  },
  {
    id: 9,
    ...
    urlFor(game) {
      return `https://www.microsoft.com/search/shop/games?q=${encodeURIComponent(game.name)}`;
    },
  },
  {
    id: 3,
    ...
    domain: "xbox.com",
    urlFor(game) {
      return `https://www.xbox.com/search?q=${encodeURIComponent(game.name)}`;
    },
  },
];
...
const findDemoGame = (idOrSlug) =>
  demoGames.find(
    (game) => game.slug === idOrSlug || String(game.id) === String(idOrSlug)
  );

const getDemoTrailerResponse = (gameId) => {
  const game = findDemoGame(gameId);
  const media = mediaByGameId[Number(gameId)];
  const preview = media?.screenshots?.[0] || game?.background_image || fallbackTrailerPreview;
...
const getFallbackTrailerResponse = (gameId, preview = fallbackTrailerPreview) => {
  return {
    count: 1,
    next: null,
    results: [
...
const buildDownloadOptions = (game) => {
  return downloadWebsites.map((website) => ({
    id: website.id,
    url: website.urlFor(game),
    website: {
      id: website.id,
      name: website.name,
      slug: website.slug,
    ...
    note: "Opens a legal website/search page for this game.",
  }));
};

const getDemoDownloadResponse = (idOrSlug) => {
  const game = findDemoGame(idOrSlug);
  if (!game) return null;
  ...
  return {
    ...
    downloads: buildDownloadOptions(game),
  };
};
...
const getDemoResponse = (endpoint, query, id) => {
  ...
  if (/^\/games\/\d+\/movies$/.test(endpoint)) {
    ...
    return getDemoTrailerResponse(gameId);
  }
...
const normalizeLiveDownloadResponse = (game) => {
  return {
    game: {
      id: game?.id,
      name: game?.name,
    ...
    downloads: game?.name
      ? buildDownloadOptions(game)
      : [
          {
            id: 0,
            url: "https://www.google.com/search?q=official+game+download",
            website: fallbackDownloadWebsite,
            note: "Opens a legal official-download search.",
          },
        ],
  };
};

const buildFallbackDownloadResponse = (idOrSlug) => {
  const fallbackName = idOrSlug
    .toString()
    .replace(/-/g, " ")
...
const handleDownloadRequest = async (req, res) => {
  try {
    if (!apiKey) {
      const demo = getDemoDownloadResponse(req.params.id);
      return res.json(demo || buildFallbackDownloadResponse(req.params.id));
    }

    const gameResponse = await instance.get(`/games/${req.params.id}`);

    res.json(normalizeLiveDownloadResponse(gameResponse.data));
  } catch (err) {
    ...
    if (err.response?.status === 404) {
      return res.json(buildFallbackDownloadResponse(req.params.id));
    }
...
const handleTrailerRequest = async (req, res) => {
  try {
    if (!apiKey) {
      return res.json(getDemoTrailerResponse(req.params.id));
    }

    const response = await instance.get(`/games/${req.params.id}/movies`);
    ...
    const gameResponse = await instance.get(`/games/${req.params.id}`);
    res.json(
      getFallbackTrailerResponse(req.params.id, gameResponse.data?.background_image)
    );
  } catch (err) {
    if (apiKey && err.response?.status === 404) {
      try {
        const gameResponse = await instance.get(`/games/${req.params.id}`);
        return res.json(
          getFallbackTrailerResponse(req.params.id, gameResponse.data?.background_image)
        ...
        return res.json(getFallbackTrailerResponse(req.params.id));
      }
    }
...
const handleCollectionRequest = async (req, res) => {
  try {
    ...
    if (/^\/games\/\d+\/movies$/.test(endpoint) && !response.data?.results?.length) {
      const gameId = endpoint.split("/")[2];
      const gameResponse = await instance.get(`/games/${gameId}`);
      return res.json(
        getFallbackTrailerResponse(gameId, gameResponse.data?.background_image)
      );
    }
...
app.get(`${API_PREFIX}/downloads/:id`, handleDownloadRequest);
app.get(`${API_PREFIX}/trailers/:id`, handleTrailerRequest);
app.get(`${API_PREFIX}/:id`, handleItemRequest);
app.get(LEGACY_API_PREFIX, handleCollectionRequest);
app.get(`${LEGACY_API_PREFIX}/downloads/:id`, handleDownloadRequest);
app.get(`${LEGACY_API_PREFIX}/trailers/:id`, handleTrailerRequest);
